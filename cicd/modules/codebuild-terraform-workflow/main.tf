...
module "cloudwatch_logs_destroy" {
  count          = var.tf_destroy_build_spec != null ? 1 : 0
  source         = "../cloudwatch/"
  component_name = var.component_name
  project_name   = var.project_name
  tf_step        = "destroy"
...
resource "aws_codebuild_project" "this_destroy" {
  count      = var.tf_destroy_build_spec != null ? 1 : 0
  depends_on = [var.codebuild_role]

  artifacts {
    encryption_disabled    = "false"
    name                   = "${var.component_name}-destroy"
    override_artifact_name = "false"
    packaging              = "NONE"
    type                   = "CODEPIPELINE"
  }
  ...
  logs_config {
    cloudwatch_logs {
      status     = "ENABLED"
      group_name = module.cloudwatch_logs_destroy[0].log_name
    }
  ...
  source {
    buildspec           = var.tf_destroy_build_spec
    git_clone_depth     = "0"
    insecure_ssl        = "false"
    report_build_status = "false"
    type                = "CODEPIPELINE"
