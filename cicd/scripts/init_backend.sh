...
BUCKET_NAME="demo-infra-tf-state-backend"
...
destroy_s3_bucket() {
    ...
    local object_count=$(aws s3api list-object-versions --bucket "$BUCKET_NAME" --query 'length(Versions[])' --output text 2>/dev/null || echo "0")
    local delete_marker_count=$(aws s3api list-object-versions --bucket "$BUCKET_NAME" --query 'length(DeleteMarkers[])' --output text 2>/dev/null || echo "0")
...
EOF
    echo
    print_status "Example for your current ECR repos configuration:"
    echo
    cat <<EOF
terraform {
  ...
  backend "s3" {
    bucket         = "$BUCKET_NAME"
    key            = "dev/configurations/ecr-repos/terraform.tfstate"
    region         = "$AWS_REGION"
    dynamodb_table = "$DYNAMODB_TABLE"
    encrypt        = true
  }
...
show_status() {
    ...
    if aws s3api head-bucket --bucket "$BUCKET_NAME" 2>/dev/null; then
        ...
        # Get bucket details
        local versioning=$(aws s3api get-bucket-versioning --bucket "$BUCKET_NAME" --query 'Status' --output text 2>/dev/null || echo "None")
        local encryption=$(aws s3api get-bucket-encryption --bucket "$BUCKET_NAME" --query 'ServerSideEncryptionConfiguration.Rules[0].ApplyServerSideEncryptionByDefault.SSEAlgorithm' --output text 2>/dev/null || echo "None")
